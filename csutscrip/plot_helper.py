...
def plot2D(x_arr, y_arr, z_complex, xlabel, ylabel):
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    avg_abs, avg_angle = np.abs(z_complex), np.angle(z_complex)
    for i, d in enumerate([avg_abs, avg_angle]):
        if i == 0:
            pcm = axes[i].pcolormesh(x_arr, y_arr, d, shading="Auto")
        else:
            pcm = axes[i].pcolormesh(
                x_arr, y_arr, np.unwrap(d), shading="Auto", cmap="bwr")
