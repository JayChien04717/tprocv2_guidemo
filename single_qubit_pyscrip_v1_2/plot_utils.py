...
def liveplot(self, expfun, py_avg, xpts, ypts=None):
    marker_style = {
        "marker": "o",
        "markersize": 5,
        "alpha": 0.7,
    ...
    for i in tqdm(range(py_avg), desc="average count"):
        self.iq_list = expfun.acquire(self.soc, soft_avgs=1, progress=False)
...
def plot_final(xpts, data: np.ndarray, x_label: str, fitfunc, simfunc):
    marker_style = {
        "marker": "o",
        "markersize": 5,
        "alpha": 0.7,
    ...
    best_result = fitter.get_best_fit(data, fitfunc=None)
    fit_params, fit_err, best_measure = best_result

    fig, axs = plt.subplots(2, 2, figsize=(10, 6))

    ax = axs.flatten()
    for idx, measure in enumerate(("amps", "phase", "avgi", "avgq")):
        ax[idx].plot(data["xpts"], data[measure], **marker_style)
        ax[idx].set_xlabel(x_label)
        ax[idx].set_ylabel(f"{measure} (ADC unit)")
        if f"fit_{measure}" in data:
            ax[idx].plot(
                data["xpts"],
                simfunc(data["xpts"], *data[f"fit_{measure}"]),
            )
        ax[idx].set_title(f"{measure}")

    ax[3].set_title(f"Best fit: {measure}")
    ax[3].plot(
        data["xpts"],
        data[best_measure],
        **marker_style,
...
def plot_final2(
    xpts, data: np.ndarray, x_label: str, fitfunc, simfunc, return_ax=False
):
    marker_style = {
        "marker": "o",
        "markersize": 5,
...
def plot_final2_plotly(xpts, data: np.ndarray, x_label: str, fitfunc, simfunc):
    """
    Pure Plotly version of plot_final2.
    Returns fit_params, fit_err, fig
    """
    ...
    # Run fits for each measure
    for measure in ("amps", "phase", "avgi", "avgq"):
        popt, pcov, _ = fitfunc(data_dict["xpts"], data_dict[measure])
        data_dict[f"fit_{measure}"] = popt
        data_dict[f"fit_err_{measure}"] = pcov

    # Get best fit parameters (use your existing fitter)
    ...
    fig = make_subplots(
        ...
        subplot_titles=("amps", "phase", "avgi", "avgq", f"Best fit: {best_measure}"),
    ...
    for i, measure in enumerate(measures):
        ...
        if f"fit_{measure}" in data_dict:
            fig.add_trace(
                go.Scatter(
                    x=data_dict["xpts"],
                    y=simfunc(data_dict["xpts"], *data_dict[f"fit_{measure}"]),
                    mode="lines",
                    name=f"{measure} fit",
                    line=dict(dash="dash"),
                    showlegend=False,
        ...
        fig.update_yaxes(title_text=f"{measure} (ADC unit)", row=row, col=col)

    # Big panel (best measure)
    fig.add_trace(
        go.Scatter(
            x=data_dict["xpts"],
            y=data_dict[best_measure],
            mode="lines+markers",
            name=f"{best_measure} data",
        ),
        row=1,
        col=3,
    )
    fig.add_trace(
        go.Scatter(
            x=data_dict["xpts"],
            y=simfunc(data_dict["xpts"], *fit_params),
            mode="lines",
            name="best fit",
            line=dict(color="red"),
        ),
    ...
    fig.update_yaxes(title_text="ADC unit", row=1, col=3)

    fig.update_layout(
        title=f"Fit results — best measure: {best_measure}",
        autosize=True,
