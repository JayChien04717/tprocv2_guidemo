...
for key in ["spinecho", "config", "spinecho_fig", "fig"]:
...
class SpinEchoge:
    ...
    def run(self, reps):
        ...
        self.iq_list = prog.acquire(
            st.session_state.soc, soft_avgs=py_avg)
        self.t = (prog.get_time_param('wait1', "t", as_array=True) +
    ...
    def plot(self, fit=False):
        ...
        if self.iq_list is not None:
            ...
            st.session_state.spinecho_fig = fig
...
if st.button("Run"):
    ...
    st.session_state.spinecho.run(reps=st.session_state.config['reps'])
    st.success("Experiment completed!")
    st.session_state.spinecho.plot(fit=st.session_state.fit_checkbox)

if "spinecho" in st.session_state and st.session_state.spinecho:

    st.session_state.spinecho.plot(fit=st.session_state.fit_checkbox)

    if st.session_state.spinecho_fig:
        st.write(f"### Last Measurement Time: {st.session_state.timetag}")
        st.pyplot(st.session_state.spinecho_fig)
