...
def amprabi_analyze(x: int, y: float, fit: bool = True, normalize: bool = False, protocal: str = None, debug=False):
    ...
    if fit:
        ...
        if protocal=='pi':
            ...
            plt.axvline(pi_gain, color='r', ls='--',
                            label=f'pi gain = {pi_gain:.3f}')
            plt.axvline(pi2_gain, color='r', ls='--',
...
def lengthrabi_analyze(x: float, y: float, fit: bool = True, normalize: bool = False):
    ...
    if fit:
        ...
        plt.plot(x, decaysin(x, *pOpt), label='Fit')
        plt.axvline(pi_len, color='r', ls='--',
                        label=f'pi length = {pi_len:.3f} us')
        plt.axvline(pi2_len, color='r', ls='--',
...
def rabichevron(x, y, data):
    ...
    pi_gain = (x[np.argmin((np.abs(pi)))] +
               x[np.argmax((np.abs(pi)))])//2
...
def T2fring_analyze(x: float, y: float, fit: bool = True, normalize: bool = False, prefix=None):
    """T2 ramsey analyze
...
def T2decay_analyze(x: float, y: float, fit: bool = True, normalize: bool = False, prefix:str='Echo'):
    """_summary_
