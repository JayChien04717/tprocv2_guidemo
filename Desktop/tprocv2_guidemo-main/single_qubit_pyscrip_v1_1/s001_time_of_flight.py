...
class TOF:
    ...
    def run(self):
        ...
        self.iq_list = prog.acquire_decimated(
            self.soc, soft_avgs=self.cfg['soft_avgs'])
    ...
    def plot(self):
        if self.iq_list is not None:
            ...
            plt.axvline(self.t[np.argmax(np.abs(self.iq_list[0].dot([1,1j]))> 1.5*mean)], c='r', ls='--')
            plt.title(f'Time of Flight Experiment, trig = {round(self.t[np.argmax(np.abs(self.iq_list[0].dot([1,1j]))> 1.5*mean)],2)} us')
