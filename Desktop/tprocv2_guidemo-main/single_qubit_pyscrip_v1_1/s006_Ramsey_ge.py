...
class Ramsey:
    ...
    def plot(self):
        self.t2r = T2fring_analyze(self.delay_times, self.iq_list[0][0].dot([1,1j]), prefix='Ramsey')

    def correct_detune(self):
        if abs(self.t2r[1]-self.cfg['ramsey_freq'])>0.2:
            self.cfg['qubit_freq_ge'] = self.cfg['qubit_freq_ge']-round((self.t2r[1]-self.cfg['ramsey_freq']),2)
            print(f'over detune {round((self.t2r[1]-self.cfg["ramsey_freq"]),2)}')
        else:
            pass
    ...
    def saveLabber(self, qb_idx):
        expt_name = "006_Ramsey_ge" + f"_Q{qb_idx}"
