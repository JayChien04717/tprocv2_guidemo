...
class SingleShot_ge_opt:
    ...
    def run(self, SHOTS):
        ...
        self.freq_steps = 20
        self.freq_step_size = 0.2  # [MHz]
        self.freq_start = (self.cfg["res_freq_ge"]) - (
            (self.freq_steps / 2) * self.freq_step_size
        )

        self.gain_steps = 18
        self.gain_step_size = 0.05  # [DAC units]
        self.gain_start = 0.05

        self.length_steps = 20
        self.length_step_size = 0.2  # [us units]
        self.length_start = 0.5
        ...
        for length_idx in tqdm(range(self.length_steps), desc="Length loop"):
            I_g_length = []
            Q_g_length = []
            I_e_length = []
            Q_e_length = []
            for gain_idx in tqdm(range(self.gain_steps), desc="Gain loop", leave=False):
                I_g_gain = []
                Q_g_gain = []
                I_e_gain = []
                Q_e_gain = []
                for freq_idx in tqdm(
                    range(self.freq_steps), desc="Freq loop", leave=False
                ):
                    self.cfg.update(
                        {
                            "steps": SHOTS,
                            "res_freq_ge": self.freq_start
                            + self.freq_step_size * freq_idx,
                            "res_gain_ge": self.gain_start
                            + self.gain_step_size * gain_idx,
                            "ro_lengths": self.length_start
                            + self.length_step_size * length_idx,
                        }
                    )
                    ...
                    iq_list_g = ssp_g.acquire(self.soc, soft_avgs=1, progress=False)
                    ...
                    iq_list_e = ssp_e.acquire(self.soc, soft_avgs=1, progress=False)
        ...
        self.I_g_array = np.array(I_g_array)
        self.Q_g_array = np.array(Q_g_array)
        self.I_e_array = np.array(I_e_array)
        self.Q_e_array = np.array(Q_e_array)
    ...
    def analyze(self):
        fid_Array = np.zeros((self.length_steps, self.gain_steps, self.freq_steps))

        for length_idx in tqdm(range(self.length_steps), desc="Analyze max fidelity"):
            for gain_idx in range(self.gain_steps):
                for freq_idx in range(self.freq_steps):
                    I_g = self.I_g_array[length_idx][gain_idx][freq_idx]
                    Q_g = self.Q_g_array[length_idx][gain_idx][freq_idx]
                    I_e = self.I_e_array[length_idx][gain_idx][freq_idx]
                    Q_e = self.Q_e_array[length_idx][gain_idx][freq_idx]
        ...
        max_length = self.length_start + self.length_step_size * max_length_idx
        max_gain = self.gain_start + self.gain_step_size * max_gain_idx
        max_freq = self.freq_start + self.freq_step_size * max_freq_idx

        print(f"Max fidelity: {np.max(fid_Array):.4f}")
        print(
            f"At length={max_length:.3f} us, gain={max_gain:.3f} DAC, freq={max_freq:.3f} MHz"
        )

        return round(max_length, 3), round(max_gain, 6), round(max_freq, 6), fid_Array
