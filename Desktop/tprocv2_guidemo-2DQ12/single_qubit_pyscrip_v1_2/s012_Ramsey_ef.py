...
class Ramsey_ef:
    ...
    def correct_detune(self):
        if abs(self.t2r[1]-self.cfg['ramsey_freq'])>0.005:
            self.cfg['qubit_freq_ef'] = self.cfg['qubit_freq_ef']-round((self.t2r[1]-self.cfg['ramsey_freq']),2)
    ...
    def liveplot(self, py_avg):
        ...
        ax.set_title(f'T2 Ramsey ef = {self.t2r[3]:.2f}$\mu s, detune = {self.t2r[1]:.5f}MHz \pm {(error[1])*1e3:.3f}kHz$', fontsize=15)
        ax.legend()
