...
def select_config_idx(
    *configs: Dict[str, Any], idx: Optional[int] = None
) -> Dict[str, Any]:
    """
        ...
        for key, value in config.items():
            if isinstance(value, list):
                selected_config[key] = (
                    value
                    if idx is None
                    else (value[idx] if idx < len(value) else value[0])
                )
            else:
...
def get_next_filename_labber(dest_path: str, exp_name: str) -> str:
    ...
    existing_files = [
        ...
        if re.match(rf"{re.escape(exp_name)}_\d+\.hdf5", f)
    ]
    next_index = (
        max([int(re.search(r"_(\d+)", f).group(1)) for f in existing_files], default=0)
        + 1
    )
...
def saveshot(
    ...
    with h5py.File(file_path, "a") as f:
        ...
        for key, value in data_dict.items():
            group.create_dataset(key, data=value, compression="gzip", overwrite=True)
