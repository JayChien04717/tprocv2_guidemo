...
for key in ["powerrabi", "powerrabi_fig"]:
...
class PowerRabi:
    ...
    def run(self, py_avg, fit=False):
        ...
        for i in range(py_avg):
            ...
            ax.cla()
            ax.plot(self.gains, np.abs(self.iqdata), **marker_style, label="Magnitude")
    ...
    def save_labber(self):
        if self.gains is None or self.iq_list is None:
        ...
        hdf5_generator(
            filepath=path,
            x_info={"name": "Gain", "unit": "a.u", "values": self.gains},
            z_info={
                "name": "Signal",
                "unit": "a.u.",
                "values": self.iqdata,
...
with col1:
    start_gain = st.number_input(
        "Start Gain (a.u)", min_value=0.0, max_value=1.0, value=0.0, step=0.1
    )
with col2:
    stop_gain = st.number_input(
        "Stop Gain (a.u)", min_value=start_gain, max_value=1.0, value=0.5, step=0.1
    )
with col3:
...
if st.button("Run"):
    ...
    st.session_state.powerrabi.run(py_avg=pyavg, fit=st.session_state.fit_checkbox)
    st.success("Experiment completed!")

# -------------------------- Plot Result -------------------------- #
if (
    st.session_state.powerrabi
    and hasattr(st.session_state.powerrabi, "iqdata")
    and st.session_state.powerrabi.iqdata is not None
):
    gains = st.session_state.powerrabi.gains
    iqdata = st.session_state.powerrabi.iqdata

    fig, ax = plt.subplots(figsize=(12, 5))
    ax.plot(gains, np.abs(iqdata), label="Magnitude", marker="o", markersize=5)

    if st.session_state.fit_checkbox:
        pOpt, _ = fitter.fitdecaysin(gains, np.abs(iqdata))
        ax.plot(gains, fitter.decaysin(gains, *pOpt), label="Fit")
        pi, pi2 = pipulse_analyze(pOpt)
        ax.axvline(pi, ls="--", c="red", label=f"$\\pi$ = {pi:.4f} a.u")
        ax.axvline(pi2, ls="--", c="green", label=f"$\\pi/2$ = {pi2:.4f} a.u")
    st.session_state.pi = pi
    st.session_state.pi2 = pi2
    ax.set_xlabel("Gain (DAC unit)")
...
# -------------------------- Update Config -------------------------- #
if st.button("Update pi/pi2 gain"):
    if st.session_state.get("pi") is not None:
        pi, pi2 = st.session_state.pi, st.session_state.pi2
        sync_param_to_config(
            "qubit_pi_gain_ge", round(pi, 4), target_cfg_group="qubit_cfg"
        )
        sync_param_to_config(
            "qubit_pi2_gain_ge", round(pi2, 4), target_cfg_group="qubit_cfg"
        )
        st.success(f"Updated qubit config with pi/pi2 gains: {pi:.4f}, {pi2:.4f}")
    else:
...
with col2:
    if st.button("SaveLabber"):
        st.session_state.powerrabi.save_labber()
