...
from sidebar_config import show_sidebar_config
...
if st.button("Connect to yoko"):
    ...
    st.session_state.yoko_global = client.connectToInstrument(
        "Yokogawa GS200 DC Source", dict(interface="USB", address=str(yoko_connect))
    )
    yoko_global_cfg = {
...
class ResonatorOnetoneFlux:
    ...
    def run(self, reps):
        ...
        total_steps = len(
            np.linspace(
                st.session_state.start_current,
                st.session_state.stop_current,
        ...
        for idx, i in tqdm(
            enumerate(self.current),
            total=total_steps,
            desc="Running",
            unit="step",
            dynamic_ncols=True,
        ):
            st.session_state.yoko_global.setValue(sQuant="Current", value=i, rate=10e-6)

            iq = prog.acquire(st.session_state.soc, soft_avgs=py_avg, progress=False)
    ...
    def save_labber(self):
        ...
        hdf5_generator(
            ...
            y_info={"name": "Current", "unit": "A", "values": self.current},
            z_info={"name": "Signal", "unit": "a.u.", "values": np.array(self.iq_list)},
...
with col3:
    st.session_state.current_steps = st.number_input(
        "Current Steps:", min_value=1, max_value=10000, value=101, step=1
    )
...
pyavg = st.number_input(
    "Soft average #:", min_value=1, max_value=10000, value=100, step=1
)
...
qubit_index = int(st.session_state.get("QubitIndex", 1))  # Default to 1 if not set
...
if st.sidebar.button("Update Config"):
    ...
    st.session_state.config[config_key] = (
    ...
    st.session_state.qubit_cfg = {
        k: v
        for k, v in st.session_state.config.items()
        if k in st.session_state.qubit_cfg
    }
    st.session_state.expt_cfg = {
        k: v
        for k, v in st.session_state.config.items()
        if k in st.session_state.expt_cfg
    }
