...
class plotting(object):
	...
	def plotall(self):
		...
		axs[0, 1].plot(f_GHz, np.abs(self.z_data_raw), label='rawdata')
		axs[0, 1].plot(f_GHz, np.abs(self.z_data_sim), label='fit')
		axs[0, 1].set_xlabel('f (GHz)')
		axs[0, 1].set_ylabel('|S21|')
		axs[0, 1].set_title('Magnitude Plot')
		axs[0, 1].legend(loc='upper right')

		axs[1, 0].plot(f_GHz, np.angle(self.z_data_raw), label='rawdata')
		axs[1, 0].plot(f_GHz, np.angle(self.z_data_sim), label='fit')
