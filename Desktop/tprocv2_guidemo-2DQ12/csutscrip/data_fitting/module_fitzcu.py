...
def T2fring_analyze(
    ...
    plt.title(
        f"T2 {prefix} = {pOpt[3]:.2f}$\mu s, detune = {pOpt[1]:.2f}MHz \pm {(error[1]) * 1e3:.2f}kHz$",
        fontsize=15,
    )
...
def T2decay_analyze(
    x: float, y: float, fit: bool = True, normalize: bool = False, prefix: str = "Echo"
):
    """_summary_
