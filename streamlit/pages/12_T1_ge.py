...
for key in ["t1", "t1_fig", "t1_fit_T1"]:
...
with col2:
    stop_t = st.number_input("Stop Time (us)", min_value=start_t, value=50.0, step=0.1)
with col3:
...
# -------------------------- Run Experiment -------------------------- #
if st.button("Run"):
    st.session_state.t1 = T1ge(st.session_state.soccfg, st.session_state.config)
    st.session_state.t1.run(py_avg=pyavg, fit=st.session_state.fit_checkbox)
    st.success("Experiment completed!")

# -------------------------- Plot Result -------------------------- #
if (
    st.session_state.t1
    and hasattr(st.session_state.t1, "iqdata")
    and st.session_state.t1.iqdata is not None
):
    t = st.session_state.t1.t
    iqdata = st.session_state.t1.iqdata

    fig, ax = plt.subplots(figsize=(12, 5))
    ax.plot(t, np.abs(iqdata), label="Magnitude", marker="o", markersize=5)

    if st.session_state.fit_checkbox:
        pOpt, _ = fitter.fitexp(t, np.abs(iqdata))
        ax.plot(t, fitter.expfunc(t, *pOpt), label=f"Fit T1={pOpt[3]:.2f} us")
        st.session_state.t1_fit_T1 = pOpt[3]
    else:
        st.session_state.t1_fit_T1 = None
...
with col2:
    if st.button("SaveLabber"):
        st.session_state.t1.save_labber()
