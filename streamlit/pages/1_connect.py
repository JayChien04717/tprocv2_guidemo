...
if st.button("Load Config"):
    ...
    st.session_state.cooling_cfg = cooling_cfg
...
if st.session_state.config_loaded:
    ...
    with st.sidebar.expander("📡 Readout Config"):
        st.json(
            select_config_idx(
                st.session_state.readout_cfg, idx=int(st.session_state.QubitIndex)
            )
        )
    ...
    with st.sidebar.expander("⚛️ Qubit Config"):
        st.json(
            select_config_idx(
                st.session_state.qubit_cfg, idx=int(st.session_state.QubitIndex)
            )
        )
    with st.sidebar.expander("❄️ Cooling Config"):
        st.json(
            select_config_idx(
                st.session_state.cooling_cfg, idx=int(st.session_state.QubitIndex)
            )
        )
