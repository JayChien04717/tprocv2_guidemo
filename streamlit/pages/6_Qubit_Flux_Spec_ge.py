...
st.title("Qubit ge Flux Spectroscopy")
...
st.session_state.expt_name = "003_qubit_flux_spec_ge"
...
for key in ["twotoneflux", "config", "twotoneflux_fig", "fig"]:
    if key not in st.session_state:
        st.session_state[key] = None


class QubitTwotoneFlux:
...
if "twotoneflux" in st.session_state and st.session_state.twotoneflux:
    ...
    if st.session_state.twotoneflux_fig:
        ...
        st.pyplot(st.session_state.twotoneflux_fig)
    ...
    with col1:
        if st.button("Save"):
            st.session_state.twotoneflux.save()
            st.success("Data saved successfully!")
    with col2:
        if st.button("SaveLabber"):
            st.session_state.twotoneflux.save_labber()
