...
from single_qubit_pyscrip.SQ007_SpinEcho_ge import SpinEchoProgram
from qick.asm_v2 import QickSweep1D

# -------------------------- Initialization -------------------------- #
st.title("SpinEcho ge")
st.session_state.expt_name = "007_SpinEcho_ge"
...
for key in ["spinecho", "spinecho_fig", "spinecho_fit_T2"]:
...
class SpinEchoge:
    ...
    def run(self, py_avg, fit=False):
        ...
        t1 = prog.get_time_param("wait1", "t", as_array=True)
        t2 = prog.get_time_param("wait2", "t", as_array=True)
        self.t = t1 + t2
        iq = 0
...
with col2:
    stop_t = st.number_input("Stop Time (us)", min_value=start_t, value=15.0, step=0.1)
with col3:
...
Ramsey_frequency = st.number_input(
    "Ramsey frequency (MHz):", min_value=0.0, max_value=10.0, value=0.5, step=0.1
)
...
# -------------------------- Run Experiment -------------------------- #
if st.button("Run"):
    st.session_state.spinecho = SpinEchoge(
        st.session_state.soccfg, st.session_state.config
    )
    st.session_state.spinecho.run(py_avg=pyavg, fit=st.session_state.fit_checkbox)
    st.success("Experiment completed!")

# -------------------------- Plot Result -------------------------- #
if (
    st.session_state.spinecho
    and hasattr(st.session_state.spinecho, "iqdata")
    and st.session_state.spinecho.iqdata is not None
):
    t = st.session_state.spinecho.t
    iqdata = st.session_state.spinecho.iqdata
    ...
    if st.session_state.fit_checkbox:
        ...
        ax.plot(t, fitter.decaysin(t, *pOpt), label=f"Fit T2={pOpt[3]:.3f} us")
        st.session_state.spinecho_fit_T2 = pOpt[3]
    else:
        st.session_state.spinecho_fit_T2 = None

    ax.set_xlabel("Time (us)")
    ax.set_ylabel("ADC unit (a.u)")
    ax.set_title("SpinEcho ge")
    ax.legend()
    st.pyplot(fig)
...
with col1:
    if st.button("Save"):
        st.session_state.spinecho.save()
        st.success("Data saved successfully!")
with col2:
    if st.button("SaveLabber"):
        st.session_state.spinecho.save_labber()
