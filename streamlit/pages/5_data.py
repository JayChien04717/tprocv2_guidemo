...
def plot_data(data: Dict[str, Any], plot_type: str = "magnitude", enable_cursor: bool = True, swap_xy: bool = False):
    ...
    if data["y_name"] and data["y_value"] is not None:
        ...
        fig = go.Figure(data=go.Heatmap(
            z=Z, x=X[0], y=Y[:, 0], colorscale="Viridis",
            hoverongaps=False,
            hovertemplate="x: %{x}<br>y: %{y}<extra></extra>"
        ))

        fig.update_layout(
            title=f"{data['z_name']} ({plot_type}) Heatmap",
        ...
        fig.update_layout(
            title=f"{data['z_name']} ({plot_type}) vs {data['x_name']}",
...
if selected_file_path:
    ...
    st.write(f"**X-axis:** {data['x_name']}")
    if data.get("y_name"):
        st.write(f"**Y-axis:** {data['y_name']}")
    st.write(f"**Z-axis:** {data['z_name']}")
