...
def plot_final2_plotly(xpts, data: np.ndarray, x_label: str, fitfunc, simfunc):
    ...
    Returns (fit_params, fit_err_diag_sqrt, fig)
    """
    ...
    # Subplots: 2 rows × 3 cols, last col spans 2 rows
    fig = make_subplots(
        rows=2,
        cols=3,
        column_widths=[0.33, 0.33, 0.34],
        # IMPORTANT: titles order must match subplot order (1,1) (1,2) (1,3) (2,1) (2,2)
        subplot_titles=("amps", "phase", f"Best fit: {best_measure}", "avgi", "avgq"),
        specs=[[{}, {}, {"rowspan": 2}], [{}, {}, None]],
    )

    # Small panels (amps, phase, avgi, avgq) -> (1,1), (1,2), (2,1), (2,2)
    ...
    # Big panel on the right (best measure) -> (1,3) spanning 2 rows
    fig.add_trace(
        go.Scatter(
            x=data_dict["xpts"],
    ...
    # Lean margins; title set by caller
    fig.update_layout(
        autosize=True,
