...
class Ramsey:
    ...
    def liveplot(self, py_avg):
        ...
        if self.cfg["ramsey_freq"] != 0:
            ...
            fig.suptitle(
                f"T2 Ramsey = {fit_params[3]:.2f}$\mu s, detune = {fit_params[1]:.5f}MHz \pm {(error[1]) * 1e3:.3f}kHz$",
                fontsize=15,
            )
            self.t2r = fit_params
            ...
            fig.suptitle(
                f"T2 Ramsey = {fit_params[2]:.2f}$\mu s$",
                fontsize=15,
            )
        fig.tight_layout()
        ...
        fig.savefig(
            os.path.join(fig_dir, "Ramsey_ge.png"), dpi=300, bbox_inches="tight"
        )
